...
module MultiD =
    let scaleDown progress n data rate =
        ...
        let realdistMap = Map.ofSeq (Seq.map (fun ({ DataName = n1; Vector = v1 }, { DataName = n2; Vector = v2 }) -> (n1, n2), Measures.pearson v1 v2) comparisons)
        let rand = new Random()
                ...
                let newGrad, newError =
                    ...
                    Seq.fold (fun (accGrad, accErr) (d2, loc2, _, _) ->
                        if d1 <> d2 then
                            ...
                            let l = List.map (fun (grad, l1, l2) ->  grad + (((l1 - l2) / fakedist) * errorterm)) (List.ofSeq l)
                            l, errorterm
                        else 
                            accGrad, accErr) 
...
//                    totalError < lastError || rand.NextDouble() < p
            ...
            if cont then
                let dataLocGradErr = 
                    ...
                    Seq.cmap (fun (d,loc,grad,err) -> 
                        let l = List.ofSeq (Seq.zip loc grad)
        ...
        let inputs = Seq.init 100 (fun _ -> init())
        ...
        List.ofSeq (dataLocGradErr) |> List.map (fun (name,loc,_,_) -> { DataName = name; Location = loc }) 
